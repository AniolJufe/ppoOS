...
$(IMAGE_NAME).iso: limine/limine kernel programs
	rm -rf iso_root
	mkdir -p iso_root/boot
	cp -v kernel/bin/kernel iso_root/boot/
	cp -v initramfs.cpio iso_root/boot/initramfs.cpio || true
	find external -maxdepth 1 -type f -exec cp -v {} iso_root/boot/ \;
	mkdir -p iso_root/boot/limine
	cp -v limine.conf iso_root/boot/limine.conf
